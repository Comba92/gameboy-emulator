...
fn main() {
    ...
    let window = video.window("GameboyEmu", 800, 600)
        .position_centered()
        .build().unwrap();
    ...
    let mut tex = texture_creator
        .create_texture_streaming(sdl2::pixels::PixelFormatEnum::RGBA32, 200, 200)
        .unwrap();
    tex.set_scale_mode(sdl2::render::ScaleMode::Nearest);

    let mut events = sdl.event_pump().unwrap();
    let timer = sdl.timer().unwrap();
    let frame_rate = (1.0f64 / 60.0 * 1000.0).round() as u64;
