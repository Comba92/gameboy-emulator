...
impl GbEmulator {
    ...
    fn bc_indirect(&mut self) -> u8 {
        self.read8(self.cpu.bc.0)
    }
    fn de_indirect(&mut self) -> u8 {
        self.read8(self.cpu.de.0)
    }
    fn hl_indirect8(&mut self) -> u8 {
        self.read8(self.cpu.hl.0)
    }
    ...
    fn af(&mut self) -> u16 {
        ((self.cpu.a as u16) << 8) | self.cpu.f.0 as u16
    }
    fn bc(&mut self) -> u16 {
        self.cpu.bc.0
    }
    ...
    fn set_f(&mut self, val: u8) {
        self.cpu.f = Flags::from_bits(val & 0xF0)
    }
    ...
    fn set_bc_indirect8(&mut self, val: u8) {
        self.write8(self.cpu.bc.0, val);
    }
    fn set_de_indirect8(&mut self, val: u8) {
        self.write8(self.cpu.de.0, val);
    }
    fn set_hl_indirect8(&mut self, val: u8) {
        self.write8(self.cpu.hl.0, val);
    }
    ...
    fn ncarry(&mut self) -> bool {
        !self.cpu.f.carry()
    }
}

type OpGet<T> = fn(&mut GbEmulator) -> T;
type OpSet<T> = fn(&mut GbEmulator, T);
...
impl GbEmulator {
    ...
    fn push(&mut self, get: OpGet<u16>) {
        self.tick(); // one more mcycle here
        let val = get(self);
        self.stack_push(val);
    }
    ...
    fn ld_sp_offset_to_hl(&mut self) {
        let val = self.immediate8();
    ...
    fn ld_hl_to_sp(&mut self) {
        ...
        self.tick(); // this takes one mcycle more
    }

    fn adc(&mut self, get: OpGet<u8>) {
        let val = get(self);
        let res = self.cpu.a as u16 + val as u16 + self.cpu.f.carry() as u16;

        self.set_z(res as u8);
        self.cpu.f.set_neg(false);
        self.cpu.f.set_hcarry(
            ((self.cpu.a & 0xF)
                .wrapping_add(val & 0xF)
                .wrapping_add(self.cpu.f.carry() as u8))
                & 0x10
                > 0,
        );
        self.set_carry(res);
    ...
    fn sbc(&mut self, get: OpGet<u8>) {
        ...
        self.cpu.f.set_hcarry(
            ((self.cpu.a & 0xF)
                ...
                .wrapping_sub(self.cpu.f.carry() as u8))
                & 0x10
                > 0,
        );
        self.set_carry(res);
    ...
    fn daa(&mut self) {
        ...
        if self.cpu.f.hcarry() || (!self.cpu.f.neg() && self.cpu.a & 0xF > 0x9) {
            correction += 0x6;
        }

        if self.cpu.f.carry() || (!self.cpu.f.neg() && self.cpu.a > 0x99) {
            correction += 0x60;
            carry = true;
        }
    ...
    fn shift_acc<FS: Fn(u8) -> u8>(&mut self, f: FS, carry_mask: u8) {
        ...
        self.cpu.f.set_zero(false); // shift sets zero as res, but shift_acc should always set to false
    }
    ...
    fn rst(&mut self, intr_handler: u16) {
        self.tick();
        self.stack_push(self.cpu.pc);
        self.cpu.pc = intr_handler;
    }
