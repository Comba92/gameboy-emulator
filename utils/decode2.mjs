...
let file = fs.readFileSync("./utils/Opcodes.json");
...
for (let [opcode, instr] of Object.entries(unprefixed)) {
  ...
  let write =
    (instr.operands.length > 1 && !NO_WRITE.includes(name)) ||
    ["INC", "DEC"].includes(name);
  let operands = instr.operands.map((o, i) => {
    return self + (write && i == 0 ? "set_" : "") + remapOperand(name, o);
  });
...
for (let [opcode, instr] of Object.entries(cbprefixed)) {
  ...
  let write =
    (instr.operands.length > 1 && !NO_WRITE.includes(name)) ||
    ["RLC", "RRC", "RL", "SLA", "RR", "RRC", "SRA", "SRL", "SWAP"].includes(
      name,
    );
  let operands = instr.operands.map((o, i) => {
